package database.dao.dao_file.booking;
...
class DaoBookingFileTest {
    ...
    private final File file = new File("src/test/java/database/dao/dao_file/booking/bookings.bin");
    private final File fileNonExisting = new File("src/test/java/database/dao/dao_file/booking/none.bin");
    ...
    void getWithObjTest3() {
        makeEmpty();
        DaoBookingFile daoBookingFile = new DaoBookingFile(file);
        assertEquals(Optional.empty(), daoBookingFile.get(Booking.getRandom()));
    }
