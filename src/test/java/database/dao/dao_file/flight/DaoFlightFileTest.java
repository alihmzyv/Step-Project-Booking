package database.dao.dao_file.flight;
...
class DaoFlightFileTest {
    ...
    private final File file = new File("src/test/java/database/dao/dao_file/flight/flights.bin");
    private final File fileNonExisting = new File("src/test/java/database/dao/dao_file/flight/none.bin");
    ...
    void getWithObjTest3() {
        makeEmpty();
        DaoFlightFile daoFlightFile = new DaoFlightFile(file);
        assertEquals(Optional.empty(), daoFlightFile.get(Flight.getRandom(1, 168, ChronoUnit.HOURS)));
    }
