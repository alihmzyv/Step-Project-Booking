package database.dao.dao_file.user;
...
class DaoUserFileTest {
    ...
    private final File file = new File("src/test/java/database/dao/dao_file/user/users.bin");
    private final File fileNonExisting = new File("src/test/java/database/dao/dao_file/user/none.bin");
    ...
    void getWithObjTest3() {
        makeEmpty();
        DaoUserFile daoUserFile = new DaoUserFile(file);
        assertEquals(Optional.empty(), daoUserFile.get(User.getRandom()));
    }
