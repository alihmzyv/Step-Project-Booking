package database.services.dao_file.passenger;
...
class PassengerServiceFileTest {
    ...
    private final File file = new File("src/test/java/database/services/dao_file/passenger/passengers.bin");
    private final File fileNonExisting = new File("src/test/java/database/services/dao_file/passenger/none.bin");
    ...
    void getWithObjTest3() {
        makeEmpty();
        PassengerService fs = new PassengerService(new DaoPassengerFile(file));
        assertEquals(Optional.empty(), fs.getPassenger(Passenger.getRandom()));
    }
